...
contract("SimpleVault2", (accounts) => {
    ...
    before(async () => {
        ...
        this.nftB20 = await NFTB20.deployed();
        this.vault = await Vault2.deployed();
        await this.nftB20.mintTo(owner);
        assert.equal(owner, await this.nftB20.ownerOf(1));
        // owner renounces ownership over Vault
        await this.vault.renounceOwnership({ from: owner, gas: 50000 });
    });
    ...
    describe("safeAddAsset", () => {
        ...
        it("[require] - caller needs to be the nftKey owner", async () => {
            await expectRevert(
    ...
    describe("transferOwnership", () => {
        ...
        it("[revert] - even if called by key owner", async () => {
            await expectRevert(
                this.vault.transferOwnership(tester2, { from: owner, gas: 50000 }),
                "revert",
            );
        });

        it("[prevent] - add/transfer assets after key ownership transfer", async () => {
            await this.nftB20.safeTransferFrom(owner, tester1, 1);
            assert.equal(tester1, await this.nftB20.ownerOf(1));
        ...
        it("[allow] - add/transfer assets with new key owner", async () => {
            const nftTest = await NFT1.new();
            await nftTest.mintTo(tester1);
            await nftTest.approve(this.vault.address, 1, { from: tester1 });
    ...
    describe("escapeHatchERC721", () => {
        ...
        before(async () => {
            this.snapshotIds.push((await timeMachine.takeSnapshot())["result"]);
            // Mint and send nft to Vault2 contract address
            for (i = 0; i < 2; i++) {
                await this.nft1.mintTo(owner);
                await this.nft1.approve(this.vault.address, i + 1, { from: owner });
            }
