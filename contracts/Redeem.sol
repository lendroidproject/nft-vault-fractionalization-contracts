...
contract Redeem is IRedeem, Ownable {
    ...
    enum AppMode { BUYOUT_ENABLED, REDEEM_ENABLED }
    ...
    function enableRedeem(address token0Address, address token2Address,
            uint256 token2Amount) external override onlyOwner {
        require(mode == AppMode.BUYOUT_ENABLED &&
    ...
    function redeem(uint256 token0Amount) external override {
        require(mode == AppMode.REDEEM_ENABLED, "{redeem} : app mode is not REDEEM_ENABLED");
